...
extension ABPManager {
    /// Add a website the user's whitelist.
    @objc func whiteList(aWebsite website: NSString) -> Bool
    {
        guard let name = website.whitelistedHostname(), name.count != 0 else { return false }
        // DZ: Verify use of count
        ...
        return Observable.create { observer in
            ...
            session.getAllTasks { (tasks: [URLSessionTask]) in
                let websites = tasks.map {
                    ...
                    guard ABPManager.sharedInstance().whiteList(aWebsite: nsString) else { return nil }
                    return website
                }
